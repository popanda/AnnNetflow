...
*
*   For nice reading choose tab spacing with tab width 4. 
*/
...
int main(int argc, char **argv)
{
    ...
    short               retValue;
    struct ether_header *etherPtr;      //ethernet header for storing the information about coming packet - what type it is etc
    ...
    signal(SIGFPE, 	&sigHandler);		//catch when bad arithmetic information resulting in overlow
    signal(SIGILL, 	&sigHandler);		//catch when illegal instruction is detect
    signal(SIGSEGV, &sigHandler);		//catch when invalid access to storage happened
    signal(SIGTERM, &sigHandler);		//catch when atermination request sent to the program
    ...
    {
        if (DEBUG)
            SOUT << "Processing parameters failed." << SENDL;
        return 1;
    }
    ...
    {
        SERR << "Error occured while opening pcap file. The file is most likely invalid." << SENDL;
        return 1;
    }
    ...
    while ((packet = pcap_next(pcapHandle, &pktHdr)) != NULL)
    {
        ...
        if (DEBUG)
            SOUT << "\n\nPacket no. " << tcps + udps + icmps + igmps + 1 << SENDL;
        if (MORE_DEBUG)
        {
            SOUT << "\tCECKING THE INTERVAL TO EXPORT:";
            SOUT << "  pktTime:" << std::fixed << timeInSeconds(pktTime) << "  -  intervalBgn:" << std::fixed << *intervalBgn << " (" << std::fixed << timeInSeconds(pktTime) - *intervalBgn << ")  >=  " << "params.intervalToExport:" << params.intervalToExport << SENDL;
        }
        ...
        if ((timeInSeconds(pktTime) - *intervalBgn) >= params.intervalToExport)
        {
            if (DEBUG)
            {
                SOUT << "\t --> Interval to export reached!" 
                     << "\tExporting expired flows!" << SENDL;
            }

            if ((exportExpired(expiredFlowInfoVector, pktTime, intervalBgn, params)) != 0)
            {
                SERR << "Error occured while exporting to collector." << SENDL;
                return 1;
            }
        ...
        for(t_flowInfoVector::iterator iterator = flowInfoVector->begin(); iterator != flowInfoVector->end(); ) 
        {
            if (MORE_DEBUG)
            {
                ...
                SOUT << "\tpktTime:" << std::fixed << timeInSeconds(pktTime) << "  -  (*iterator)->lstPktTime:" << std::fixed << timeInSeconds((*iterator)->lstPktTime) << " (" << std::fixed << (timeInSeconds(pktTime) - timeInSeconds((*iterator)->lstPktTime)) << ")  >=  params.intervalToExpire:" << params.intervalToExpire << SENDL;
            }
            ...
            if ((*iterator)->proto == TCP_PROTO && ( (timeInSeconds(pktTime) - timeInSeconds((*iterator)->lstPktTime) ) >= params.intervalToExpire ))
            {
                ...
                if (DEBUG)
                    SOUT << "\t --> Unactive timeout reached!" << SENDL;
        ...
        if (DEBUG)
            SOUT << "\tRecieved packet time (seconds):" << pktTime.tv_sec << " \t(microseconds):" << pktTime.tv_usec << SENDL;
        ...
        if (ntohs(etherPtr->ether_type) == ETHERTYPE_IP)         
        {
            ...
            {				
                if (retValue == 1)
                    SERR << "Error while proccesing TCP/UDP/ICMP/IGMP packet" << SENDL;
                if (retValue == 2)
                    SERR << "Error while proccesing Unknown packet." << SENDL;
                return 1;
            }
        }
    ...
    if (DEBUG)
    {
        SOUT << "Number of expired flows: " << expiredFlowInfoVector->size() << SENDL;
        SOUT << "Number of flows in cache: " << flowInfoVector->size() << SENDL;
        SOUT << "End of file reached..." << SENDL;
    }
    ...
    if ((exportExpired(expiredFlowInfoVector, pktTime, intervalBgn, params)) != 0)
    {
        SERR << "Error occured while exporting." << SENDL;
        return 1;
    }
    
    SERR << "== STATISTICS =="  << SENDL;
    SERR << "Total flows: "     << flowSequence << SENDL;
    SERR << "TCP \tpackets: "   << tcps         << ",\tbytes: " << tcpsb    << SENDL;
    SERR << "UDP \tpackets: "   << udps         << ",\tbytes: " << udpsb    << SENDL;
    SERR << "ICMP \tpackets: "  << icmps        << ",\tbytes: " << icmpsb   << SENDL;
    SERR << "IGMP \tpackets: "  << igmps        << ",\tbytes: " << igmpsb   << SENDL;
    SERR << "ARP \tpackets: "   << arps         << ",\tbytes: " << arpsb    << SENDL;
    SERR << "OTHER \tpackets: " << others       << ",\tbytes: " << othersb  << SENDL;
    SERR << "total \tpackets: " << tcps + udps + icmps + igmps  
                                << ",\tbytes: " << tcpsb + udpsb + icmpsb + igmpsb << SENDL;
    
    SERR << "== END OF STATISTICS ==" << SENDL;
...
int processParams(int argc, char **argv, t_params *ptrParams)
{
    ...
    while ((a = getopt_long(argc, argv, "i:c:I:m:t:", longOptions, &optind)) != -1)
    {
        switch(a)
        {
            ...
            case 'i':
                if(DEBUG)
                    SOUT << "Option input with value " << optarg << "." << SENDL;
            ...
            case 'c':    
                ...
                {
                    //Adress processing
                    SOUT << "Address prcessing: " << ptr << SENDL;
                    if ((inet_aton(ptr, &(ptrParams->collectorAddr))) == 0)
                     {
                         SERR << "Invalid collector address."<< SENDL << "Exiting..." << SENDL;
                         return -1;
                    }
                }
                ptr = strtok(NULL, ":");
                if(ptr)
                {
                    SOUT << "Port processing: " << ptr << SENDL;
                    ptrParams->collectorPort = strtol(ptr, &endPtr, 10);
                    if (*endPtr != '\0')
                    {
                        SERR << "Invalid value for collector port." << SENDL << "Exiting..." << SENDL;
                        return -1;
                    }	
                }
                if(DEBUG)
                    SOUT << "Option collector with value " << optarg << "." << SENDL;
                break;

            case 'I':
                if(DEBUG)
                    SOUT << "Option interval with value " << optarg << "." << SENDL;
                ptrParams->intervalToExport = strtol(optarg, &endPtr, 10);
                if (*endPtr != '\0')
                {
                    SERR << "Invalid value for interval to export." << SENDL << "Exiting..." << SENDL;
                    return -1;
                }	
                break;
            ...
            case 'm':
                if(DEBUG)
                    SOUT << "Option max-flows with value " << optarg << "." << SENDL;
                ptrParams->maxFlows = strtol(optarg, &endPtr, 10);
                if (*endPtr != '\0')
                {
                    SERR << "Invalid value for max-flows." << SENDL << "Exiting..." << SENDL;
                    return -1;
                }
                break;
            ...
            case 't':
                if(DEBUG)
                    SOUT << "Option tcp-timeout with value " << optarg << "." << SENDL;
                ptrParams->intervalToExpire = strtol(optarg, &endPtr, 10);
                if (*endPtr != '\0')
                {
                    SERR << "Invalid value for interval to expire (tcp-timeout)." << SENDL << "Exiting..." << SENDL;
                    return -1;
                }
                break;

            case '?':
                SERR << "Invalid option." << SENDL << "Exiting..." << SENDL;
                if (DEBUG)
                    SOUT << "*?*" << SENDL;
                return -1;
                break;
    ...
    if (optind < argc)
        {
            SERR << "Unknown argument." << SENDL << "Exiting..." << SENDL;
            if (DEBUG)
                SOUT << "*optInd < argc*" << SENDL;
            return -1;
        }
...
int processTCP(const u_char *packet, t_flowInfoVector *flowInfoVector, t_flowInfoVector *expiredFlowInfoVector, struct ip *myIP, struct timeval pktTime, bpf_u_int32 len, t_params params)
{
    ...
    if (DEBUG)
    {
        SOUT << "\tTCP (" << inet_ntoa(myIP->ip_src) << ":" << u_short(myTCP->th_sport) << " --> "  
             << inet_ntoa(myIP->ip_dst) << ":" << u_short(myTCP->th_dport) << ") PROTOCOL PROCESSING." << " FLAGS: " << flags << SENDL;	
    }
    ...
    for(t_flowInfoVector::iterator iterator = flowInfoVector->begin(); iterator != flowInfoVector->end(); ++iterator) 
    {
        if (isEqualFlow(pktInfo, *iterator))
        {
            ...
            if (DEBUG)
                SOUT << "\tThis packet is part of already existing flow!" << SENDL;
        ...
        for(t_flowInfoVector::iterator iterator = flowInfoVector->begin(); iterator != flowInfoVector->end(); ++iterator) 
        {
            if (isOppositeFlow(pktInfo, *iterator))
            {
                ...
                if ((*iterator)->oneAckToExp)
                {
                    ...
                    flowInfoVector->erase(iterator);
                    if (DEBUG)
                        SOUT << "\tACK flag founded after FIN --> Adding opposite flow to expired, erasing from cache..." << SENDL;
    ...
    if (needNewFlow)
    {
        ...
        if (flowInfoVector->size() >= params.maxFlows)
        {
            if (DEBUG)
            {
                SOUT << "\t --> Max-flows in cache reached!" 
                     << "\tExpiring the oldest unactive flow!" << SENDL;
            }
            expireOldestUnactive(flowInfoVector, expiredFlowInfoVector, pktTime);
            if (params.maxFlows == 0)
            {
                //no space in cache, this flow has to be expired right after its creation
                expireImmediately = true;
            }
        }
        ...
        if (DEBUG)
        {
            SOUT << "\tNew flow added: "<< inet_ntoa(currentFlow->srcAddr) << ":" << currentFlow->srcPort << " --> "  
             << inet_ntoa(currentFlow->dstAddr) << ":" << currentFlow->dstPort << " Flows in cache now: " << flowInfoVector->size() << " Expired flows: " << expiredFlowInfoVector->size() << SENDL;
        }
    }
...
int processUDPorICMPorIGMP(const u_char *packet, t_flowInfoVector *expiredFlowInfoVector, struct ip *myIP, struct timeval pktTime, bpf_u_int32 len)
{
    ...
    switch(myIP->ip_p)
    {
        case UDP_PROTO:	
            ...
            if (DEBUG)
            {
                SOUT << "\tUDP (" << inet_ntoa(myIP->ip_src) << " : " << u_short(myUDPorICMPorIGMP->uh_sport) << "  -->  "  << inet_ntoa(myIP->ip_dst) << " : " << u_short(myUDPorICMPorIGMP->uh_dport) << ") PROTOCOL PROCESSING." << SENDL;
            }
            break;
        ...
        case ICMP_PROTO:
            ...
            if (DEBUG)
            {
                SOUT << "\tICMP (" << inet_ntoa(myIP->ip_src) << " : " << u_short(myUDPorICMPorIGMP->uh_sport) << "  -->  "  << inet_ntoa(myIP->ip_dst) << " : " << u_short(myUDPorICMPorIGMP->uh_dport) << ") PROTOCOL PROCESSING." << SENDL;
            }
            break;
        ...
        case IGMP_PROTO:
            ...
            if (DEBUG)
            {
                SOUT << "\tIGMP (" << inet_ntoa(myIP->ip_src) << " : " << u_short(myUDPorICMPorIGMP->uh_sport) << "  -->  "  << inet_ntoa(myIP->ip_dst) << " : " << u_short(myUDPorICMPorIGMP->uh_dport) << ") PROTOCOL PROCESSING." << SENDL;
            }
            break;
    }
...
int exportExpired(t_flowInfoVector *expiredFlowInfoVector, struct timeval pktTime, double *intervalBgn, t_params params)
{
    ...
    {
        SERR << "Failed on creating packet." << SENDL;
        return -1;
    }
    ...
    for (;pktCnt;pktCnt--)
    {
        ...
        if (MORE_DEBUG)
        {
            ...
            SOUT <<  newPkt->hdr.version << " : " << newPkt->hdr.count << " : " << newPkt->hdr.SysUptime << " ("  << sysuptimeD << ") : " << newPkt->hdr.unix_secs << " : " << newPkt->hdr.unix_nsecs << " : " << newPkt->hdr.flow_sequence << " : " << newPkt->hdr.engine_type << " : " << newPkt->hdr.engine_id << " : " << newPkt->hdr.sampling_interval << SENDL;
        }
            
        //sends this packet
        if ((sendto(skt, newPkt, sizeof(t_nfV5hdr) + flowsCnt * sizeof(t_nfV5flowRec), 0, (const sockaddr *)&saddr, sizeof(saddr))) == -1)
        {
            SERR << "Error occured while sending packet to collector." << SENDL;
            return -1;
        }
        
        if (MORE_DEBUG)
            SOUT << "Packet sended successfuly!" << SENDL;
    ...
    if (MORE_DEBUG)
    {
        SOUT << "\tExporting: ..." << SENDL;
        for(t_flowInfoVector::iterator iterator = expiredFlowInfoVector->begin(); iterator != expiredFlowInfoVector->end(); ++iterator) 
        {
            SOUT << inet_ntoa((*iterator)->srcAddr) << ":" << (*iterator)->srcPort << " --> " << inet_ntoa((*iterator)->dstAddr) << ":" << (*iterator)->dstPort << SENDL;
        }
...
void expireOldestUnactive(t_flowInfoVector *flowInfoVector, t_flowInfoVector *expiredFlowInfoVector, struct timeval pktTime)
{
    if (flowInfoVector->size() > 0)
    {
        ...
        if (DEBUG)
            SOUT << "\tafter: expired: " << expiredFlowInfoVector->size() << "  cache: " << flowInfoVector->size() << SENDL;
    }
}

/*
* Function handles with signals.	
*/
void sigHandler(int s)
{
    ...
    switch(s)
    {
        case SIGINT:
            ...
            break;
        case SIGFPE:
            SERR << "[Bad arithmetic operation resulted in overflow]" << SENDL;
            break;
        case SIGILL:
            SERR << "[Illegal instruction detected]: " << SENDL;
            break;
        case SIGABRT:
            SERR << "[ABORT]: " << SENDL;
            break;
        case SIGSEGV:
            SERR << "[Invalid access to storage]: " << SENDL;
            break;
        case SIGTERM:
            SERR << "[Temination request was sent to the program]: " << SENDL;
            break;
    }
    SERR << "Invalid termination of this program, flows in cache won't be expired and expired flows which weren't exportet won't be.\nExiting..." << SENDL;
    exit(1);
}